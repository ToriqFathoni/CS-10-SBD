...
class TransactionController {
  static async createTransaction(req, res, next) {
    try {
      ...
      const totalTransaksi = transaction.total_price || transaction.total || quantity;
      const streamId = await redisClient.xadd(
        'transaction-logs', 
        '*', 
        'userId', String(user_id),
        'itemId', String(item_id),
        'total', String(totalTransaksi)
      );
      console.log(`[Redis Stream] Log transaksi berhasil dicatat dengan ID: ${streamId}`);
